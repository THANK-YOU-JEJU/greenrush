...
const { add, sprite, pos, area, anchor, z, height, vec2, scale, drawText, Color, drawRect, rgb, drawSprite, camPos } = k;
...
export const shop = () => {
    ...
    const addItem = (ItemName: string, price: number, Y_pos: number, ItemType: ITEM_TYPE) => {
        ...
        add([sprite("buy"), "Modal", anchor("center"), area(), pos(BUTTON_X_POS, Y_pos), z(shopUILayer), "buy_button", ItemType]);
    
        add([
            "Modal",
            pos(vec2(BUTTON_X_POS - 2, Y_pos)),
            anchor("center"),
            z(shopActionLayer),
            {
                draw(this: GameObj): void {
        ...
        add([sprite("coin"), anchor("center"), area(), pos(BUTTON_X_POS + 10, Y_pos), z(shopActionLayer), "Modal", scale(0.6)]);
