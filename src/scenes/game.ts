...
const GameScene = (stage: number) => {
    ...
    let canDrawTutorial: boolean = false;
    ...
    const doModalTutorial = () => {
        if (gameState == "GS_PAUSE") return;

        pause();

        add([
            rect(TUTORIAL_BKG_WIDTH, TUTORIAL_BKG_HEIGHT, { radius: 30 }),
            anchor("topleft"),
        ...
        add([
            ...
            color(255, 255, 255),
            pos(g_StagePosX + g_StageWidth * 0.5, g_StagePosY + g_StageHeight * 0.15),
            area(),
            anchor("center"),
            z(LAYER.ACTION),
            "Tutorial",
        ...
        let tutorialDrag = add([
            sprite("tutorialDrag"),
            anchor("center"),
            area(),
            pos(g_StagePosX + g_StageWidth * 0.32, g_StagePosY + g_StageHeight * 0.32),
            z(LAYER.ACTION),
            "Tutorial1",
        ]);

        let tutorialCleaner = add([
            sprite("tutorialCleaner"),
            anchor("center"),
            area(),
            pos(g_StagePosX + g_StageWidth * 0.32, g_StagePosY + g_StageHeight * 0.58),
            z(LAYER.ACTION),
            "Tutorial1",
        ]);
        ...
        add([
            text("Put the trash inside the trash can of its type. If you miss, the trash goes into the incinerator.", {
                size: 16,
                font: "LilitaOne-Regular",
                width: g_StageWidth * 0.3,
            }),
            color(255, 255, 255),
            pos(g_StagePosX + g_StageWidth * 0.6, g_StagePosY + g_StageHeight * 0.32),
            area(),
            anchor("center"),
            z(LAYER.ACTION),
            "Tutorial1",
        ...
        add([
            text("If the trash is polluted (has a radioactive sign), us the clearer before recycling it.", {
                size: 16,
                font: "LilitaOne-Regular",
                width: g_StageWidth * 0.3,
            }),
            color(255, 255, 255),
            pos(g_StagePosX + g_StageWidth * 0.6, g_StagePosY + g_StageHeight * 0.58),
            area(),
            anchor("center"),
            z(LAYER.ACTION),
            "Tutorial1",
        ...
        tutorialCleaner.play("cleaner", { speed: 70 });
        tutorialDrag.play("drag", { speed: 70 });
    };
    ...
    onClick("Background", () => {
        console.log("onClick - Background");
    });

    onClick("CloseButton", () => {
        get("*").forEach((obj) => {
            if (obj.is("Modal")) obj.destroy();
        });
        pause();
    });
    ...
    onClick("Tutorial1", () => {
        get("*").forEach((obj) => {
            if (obj.is("Tutorial1")) obj.destroy();
        });

        const tutorialParts: { sprite: string; yPoint: number; description: string }[] = [
            {
                sprite: "GPoint",
                yPoint: 0.23,
                description: "must be filled to clear the level. correct trash can: 2 points, wrong trash can: 1 point",
            },
            {
                sprite: "DPoint",
                yPoint: 0.32,
                description: "will lead to game over. miss polluted trash: 3 points, miss regular trash: 1 point",
            },
            {
                sprite: "coin",
                yPoint: 0.41,
                description: "correctly recycle the trash with the coin floating. coins can we used in the store",
            },
            {
                sprite: "autoRecycle",
                yPoint: 0.5,
                description: "automatically recycles the trash",
            },
            {
                sprite: "autoClean",
                yPoint: 0.59,
                description: "automatically cleans the polluted trash",
            },
            {
                sprite: "slowConBelt",
                yPoint: 0.68,
                description: "slows down the conveyor belt",
            },
        ];

        tutorialParts.map((item) => {
            add([
                sprite(item.sprite),
                anchor("center"),
                area(),
                pos(g_StagePosX + g_StageWidth * 0.25, g_StagePosY + g_StageHeight * item.yPoint),
                z(LAYER.ACTION),
                "Tutorial2",
            ]);
            ...
            add([
                text(item.description, {
                    size: 16,
                    font: "LilitaOne-Regular",
                    width: g_StageWidth * 0.5,
                }),
                color(255, 255, 255),
                pos(g_StagePosX + g_StageWidth * 0.55, g_StagePosY + g_StageHeight * item.yPoint),
                anchor("center"),
                area(),
                z(LAYER.ACTION),
                "Tutorial2",
    ...
    onClick("Tutorial2", () => {
        get("*").forEach((obj) => {
            if (obj.is("Tutorial2")) obj.destroy();
        });

        get("*").forEach((obj) => {
            if (obj.is("Tutorial")) obj.destroy();
        });
